...
\subsection{Fall patterns based on Esper EPL \cite{LorenaFall}}
\label{subsec:CEP}
...
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,language=SQL,caption=Fall pattern based on Kozina et al.\cite{LorenaFall,Kozina}.,label=CEPPattern, breaklines=true]
...
\end{itemize}
Referring to the selected properties this example checks, if the initial acceleration of node 1 (a1.accelS1) is $<= 9.81$ m/$s^2$ which indicates that the person is in a stationary position. Subsequently, it will be checked if the difference of the successive acceleration (a2.accelS1) and the first acceleration (a1.accelS1) within 1 second is $>= 9.81$ m/$s^2$. If this condition is fulfilled, it is an indication that the person has suffered an impact. Adding the OR disjunction the second sensor node can be integrated and the statement is able to detect a fall in case one of the node's data matches the Esper EPL query and the values of the acceleration correspond to the same person.

\subsection{IoT test event generator \cite{LorenaFall}}
\label{iotteg}

IoT-TEG~\cite{LorenaFall,Gutierrez2017,TesisGutierrez2017} is a Java-based tool which takes an event 
...
\begin{figure}[!ht]
	...
	\caption[IoT-TEG Architecture]{IoT-TEG Architecture~ \cite{LorenaFall}, \cite{TesisGutierrez2017,Gutierrez2017}.}
...
definition.
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,language=XML,caption=Event type definition example \cite{LorenaFall}.,label=eventTypeDef, breaklines=true]
...
\subsubsection{Study of the values \cite{LorenaFall}} Given that the sensor (S1) is the one that suffers the 
...
\begin{figure}[!ht]
	...
	\caption[Acceleration during FAW fall]{Acceleration comparison during FAW fall \cite{LorenaFall}.}
...
fall event in order to generate test events with IoT-TEG\cite{LorenaFall}, \cite{TesisGutierrez2017,Gutierrez2017}. As it was explained in 
...
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,language=XML,caption={Fall event type definition \cite{LorenaFall}.},label=FallEvent, breaklines=true]
<?xml version="1.0" encoding="UTF-8"?>
...
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,language=XML,caption={Rules to define a FAW fall \cite{LorenaFall}.},label=FAWFallRules, breaklines=true]
<?xml version="1.0" encoding="UTF-8"?>
...
\begin{figure}[!ht]
	...
	\caption[IoT-TEG generated FAW falls]{IoT-TEG generated FAW falls \cite{LorenaFall}.}
